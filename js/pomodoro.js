/* ═══════════════════════════════════════════════════════
   Planify — js/pomodoro.js  v3
   Pomodoro — XP hook, guest mode, sezení, historie
═══════════════════════════════════════════════════════ */

const POMO_DURATIONS = { work: 25*60, short: 5*60, long: 15*60 };
const POMO_LABELS    = { work: 'Čas pracovat', short: 'Krátká přestávka', long: 'Dlouhá přestávka' };
const POMO_COLORS    = { work: 'var(--accent)', short: 'var(--green)', long: 'var(--blue)' };
...
function _renderSessionDots() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    html += `<div class="session-dot ${done ? 'done' : ''}"></div>`;
  }
...
function updatePomoTaskSelect() {
  ...
  window.APP_DATA.tasks.filter(t => !t.done).forEach(t => {
...
document.querySelector('.pomo-modes')?.addEventListener('click', e => {
  ...
  if (!btn || pomoState.running) return;
...
function _setMode(mode) {
  ...
  _updateTimerDisplay(); _updateTimerCircle();
}
...
document.getElementById('pomoStartStop')?.addEventListener('click', () => {
  if (pomoState.running) _pauseTimer(); else _startTimer();
});
...
function _startTimer() {
  ...
  pomoState.interval = setInterval(() => {
    ...
    if (pomoState.timeLeft === 0) _timerComplete();
  }, 1000);
}

function _pauseTimer() {
  clearInterval(pomoState.interval); pomoState.interval = null; pomoState.running = false;
  const btn = document.getElementById('pomoStartStop');
...
document.getElementById('pomoSkip')?.addEventListener('click', () => {
  ...
  pomoState.timeLeft = 0; _timerComplete();
});
...
function _timerComplete() {
  clearInterval(pomoState.interval); pomoState.interval = null;
  pomoState.running = false; document.title = 'Planify';
  ...
  if (isWork) {
    ...
    const taskName = taskId ? (window.APP_DATA.tasks.find(t => t.id === taskId)?.name || '') : '';

    window.APP_DATA.pomodoroHistory.unshift({
      type: 'work', task: taskName || 'Bez úkolu',
      time: new Date().toLocaleTimeString('cs-CZ', { hour:'2-digit', minute:'2-digit' }),
      date: today(),
    });

    savePomodoroState(); _renderSessionDots(); renderPomodoroHistory();

    showToast('🍅 Pomodoro dokončeno! Čas na přestávku.', 'success', 4000);

    // XP (funguje i pro hosta — jen lokálně)
    if (typeof xpPomodoroCompleted === 'function') xpPomodoroCompleted();

    if (typeof sendBrowserNotification === 'function') {
      sendBrowserNotification('🍅 Pomodoro dokončeno!', taskName ? `Pracoval jsi na: ${taskName}` : 'Čas na přestávku.');
    }

    const sessions = window.APP_DATA.pomodoroSessions;
    setTimeout(() => {
      if (sessions % 4 === 0) { _setMode('long'); showToast('☕ Čas na dlouhou přestávku (15 min)!', 'info'); }
      else { _setMode('short'); showToast('Krátká přestávka (5 min) — odpočiňte si!', 'info'); }
    }, 800);
    ...
    savePomodoroState(); renderPomodoroHistory();
    showToast('Přestávka skončila! Čas na práci. 💪', 'info');
    if (typeof sendBrowserNotification === 'function') sendBrowserNotification('Přestávka skončila!', 'Čas se vrátit k práci.');
...
function renderPomodoroHistory() {
  const container = document.getElementById('pomodoroHistory'); if (!container) return;
  const history   = window.APP_DATA.pomodoroHistory.slice(0, 12);
  if (history.length === 0) {
    container.innerHTML = '<div class="empty-state-small">Žádná sezení zatím</div>'; return;
  }
  container.innerHTML = history.map(item => `
    <div class="pomo-history-item">
      ...
      <span style="flex:1;font-size:12px;color:var(--text-secondary);padding:0 8px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
        ${escHtml(item.task || '')}
      </span>
